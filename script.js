...
const THEME_STORAGE_KEY = "fitness_helper_theme_v1";

const themeConfig = {
  blaze: { label: "燃橙", color: "#ef6a2e" },
  midnight: { label: "暗夜金", color: "#d4a84b" },
  mist: { label: "青岚", color: "#5b9279" },
  cyber: { label: "赛博紫", color: "#7c3aed" },
  pure: { label: "素白", color: "#2c3e50" },
};

const layoutConfig = {
  classic: { label: "经典卡片" },
  dashboard: { label: "双栏仪表" },
  immersive: { label: "沉浸全宽" },
  mobile: { label: "圆润卡片" },
};
...
function loadStylePrefs() {
  try {
    ...
    if (saved) {
      ...
      if (p.theme && themeConfig[p.theme]) document.documentElement.setAttribute("data-theme", p.theme);
      if (p.layout && layoutConfig[p.layout]) document.documentElement.setAttribute("data-layout", p.layout);
    }
  } catch (e) { /* ignore */ }
}

function saveStylePrefs() {
  const theme = document.documentElement.getAttribute("data-theme") || "blaze";
  const layout = document.documentElement.getAttribute("data-layout") || "classic";
  localStorage.setItem(THEME_STORAGE_KEY, JSON.stringify({ theme, layout }));
}
...
function updateThemeOptions() {
  const cur = document.documentElement.getAttribute("data-theme") || "blaze";
  const el = document.getElementById("themeOptions");
  if (!el) return;
  el.innerHTML = Object.entries(themeConfig).map(([id, c]) =>
    `<button class="style-opt${id===cur?" is-active":""}" data-action="theme" data-id="${id}"><span class="style-opt-swatch" style="background:${c.color}"></span>${c.label}</button>`
  ).join("");
}

function updateLayoutOptions() {
  const cur = document.documentElement.getAttribute("data-layout") || "classic";
  const el = document.getElementById("layoutOptions");
  if (!el) return;
  el.innerHTML = Object.entries(layoutConfig).map(([id, c]) =>
    `<button class="style-opt${id===cur?" is-active":""}" data-action="layout" data-id="${id}">${c.label}</button>`
  ).join("");
}
...
function initStyleSwitcher() {
  ...
  if (fab && panel) {
    fab.addEventListener("click", () => panel.toggleAttribute("hidden"));
    if (closeBtn) closeBtn.addEventListener("click", () => panel.setAttribute("hidden", ""));
    document.addEventListener("click", (e) => {
      if (!panel.hasAttribute("hidden") && !panel.contains(e.target) && e.target !== fab)
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!btn) return;
    if (btn.dataset.action === "theme") setTheme(btn.dataset.id);
    else if (btn.dataset.action === "layout") setLayout(btn.dataset.id);
  });
}
